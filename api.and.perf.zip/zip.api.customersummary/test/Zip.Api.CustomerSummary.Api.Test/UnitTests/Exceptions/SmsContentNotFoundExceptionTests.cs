...
{
    public class SmsContentNotFoundExceptionTests
    {
        ...
        public SmsContentNotFoundExceptionTests()
        {
            _fixture = new Fixture();
            _innerException = new Exception("I am an inner exception");
            _expectedName = _fixture.Create<string>();
        }

        [Fact]
        public void When_Instantiation_Then_Exception_Is_Not_Null()
        {
            var target = new SmsContentNotFoundException();
        ...
        public void Given_Message_And_InnerException_When_Instantiation_Then_Exception_Should_Be_Correct()
        {
            ...
            var target = new SmsContentNotFoundException(expectedMessage, _innerException);
        ...
        {
            // Arrange
            var expect = new SmsContentNotFoundException("message", _innerException)
            {
                Name = _expectedName
            };

            // Action
            SmsContentNotFoundException actual;
            ...
            using (var deserializeMemoryStream = new MemoryStream(buffer))
            {
                ...
                actual = (SmsContentNotFoundException)formatter.Deserialize(deserializeMemoryStream);
            }
        ...
        {
            var target = new SmsContentNotFoundException("message");
