...
{
    public class LmsAccountNotFoundExceptionTests
    {
        ...
        public LmsAccountNotFoundExceptionTests()
        {
        ...
        {
            var target = new LmsAccountNotFoundException();
        ...
        {
            var target = new LmsAccountNotFoundException(_expectedAccountId);
        ...
        public void Given_Message_And_InnerException_When_Instantiation_Then_Exception_Should_Be_Correct()
        {
            ...
            var target = new LmsAccountNotFoundException(expectedMessage, _innerException);
        ...
        {
            // Arrange
            var expect = new LmsAccountNotFoundException("message", _innerException)
            {
                AccountId = _expectedAccountId
            };

            // Action
            LmsAccountNotFoundException actual;
            ...
            using (var deserializeMemoryStream = new MemoryStream(buffer))
            {
                ...
                actual = (LmsAccountNotFoundException)formatter.Deserialize(deserializeMemoryStream);
            }
