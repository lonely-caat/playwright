...
test.describe('Administration, connectors', async () => {
  ...
  test('Connectors: Box @smoke @ui', async ({ page, request }) => {
    await adminPage.navigateTo('/administration/connections/box');
    const connectorName = uuidv4();

    await page.getByRole('button', { name: 'New scan' }).click();
    await adminPage.boxFillrequiredFields(connectorName);
    await adminPage.clickSaveSnake();

    await adminPage.checkRowExists(connectorName);
    await deleteConnector(request, 'box', connectorName);
  });
});
test.describe('Administration, connectors', async () => {
  let adminPage: AdminPage;
  ...
  test('BOX: delete connection @ui', async ({ page }) => {
    await adminPage.navigateTo('/administration/connections/box');
  ...
  test('BOX: edit connection @ui', async ({ request }) => {
